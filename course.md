...
```
 - software analysis:
 **set up ()**<br>It is used primarily to write code that initializes.The setup() function is called only once after the MCU is started.<br>
 **loop ()**<br>A cyclic function.After the setup() function is completed, the MCU will call the loop() function, and the loop() function will be loop called.<br>
 **int ledPin = 5;**<br>Here, we assigned the variable name "ledPin" to pin5, that is to say, ledPin stands for pin5 on the microcontroller.<br>
 **pinMode(ledPin, OUTPUT);**<br>Set ledPin to output mode.<br>
**digitalWrite(ledPin, HIGH);**<br>When we set the ledPin as output, HIGH means sending high level to the pin, LED on.<br>
**digitalWrite(ledPin, LOW);**<br>When we set the led as output, low stands for sending low level to the pin, LED off.<br>
