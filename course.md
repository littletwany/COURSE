...
- software analysis:
  ...
  - **int ledPin = 6;**<br>Here, we assigned the variable name "ledPin" to pin5, that is to say, ledPin stands for pin6 on the microcontroller.
...
The first thing we need to know is that the input of the button is a digital signal, and there are only two states, 0 or 1, so we can control the output based on those two states.
 ...
 - Hardware connection:
   - Module connection:
     ...
     - Use a Grove cable to connect the Grove LED to Seeeduino Lotus's digital interface D5,connect the Grove Button to digital interface D9.
...
Unlike buttons, the signals entered by knobs are analog.Unlike digital signals, analog signals range in value from 0 to 1023.We can control the frequency of led flashing based on this signal.
 ...
 - Hardware connection:
   - Module connection:
     - The default onboard interface specifies:
     - use a Grove cable to connect LED to Seeeduino Lotus's digital interface D6, and a Grove cable to connect the Grove Rotary Switch to analog signal interface A0.
   - The microcontroller is then connected to the computer via a USB cable.
- Hardware analysis:
  - Input unit: knob
  - Control unit: microcontroller
...
- exercise: as the environment slowly brightens, the LED lights will lighten. As the light slowly dimmed, the LED dimmed. The LED will go from dark to light or from light to dark. To achieve this, we will use pulse width modulation.
 ...
 - Hardware connection:
   - Module connection:
     - The default onboard interface specifies:
     - Use Grove wire: Connect the Grove LED to Seeeduino Lotus's digital signal interface D6,connect the Grove Light Sensor to Seeeduino Lotus's analog signal interface A1.
   - The microcontroller is then connected to the computer via a USB cable.
...
The sound sensor can detect the sound intensity of the environment, and its output is also simulated.I'm sure you've all been exposed to the sound control lights, but now we can do one ourselves, and with the basics, this experiment will be easy for you.
 ...
 - Hardware connection:
   - Module connection:
     - The default onboard interface specifies:
     - Use Grove wire: Connect the Grove LED to Seeeduino Lotus's digital signal interface D6, Connect the Grove Sound Sensor to Seeeduino Lotus's analog signal interface A2.
   - The microcontroller is then connected to the computer via a USB cable.
- Software code: 
   - Open Arduino IDE. 
   - You can copy the code directly to Arduinoied. Although we encourage you to type the code out by hand to develop your skills.
...
Have you ever wondered about the temperature and humidity of your surroundings?Want to know the exact number?Want to wear a skirt or coat today depending on the temperature?Let's make a hygrometer!
 ...
 - Hardware connection:
   - Module connection:
     - The default onboard interface specifies:
     - Use Grove wire: Connect the OLED to Seeeduino Lotus's I2C interface (note: IIC's default address is 0x78). Connect the Grove Temperature and Humidity Sensor to Seeeduino Lotus's digital signal interface D3.
   - The microcontroller is then connected to the computer via a USB cable.
 - Software code: 
   - Open Arduino IDE. 
...
### 4. Hall Sensor - the mystery of the magnetic field
 ...
 - Module connection:
     - The default onboard interface specifies:
     - Use Grove wire: Connect the Grove Buzzer to Seeeduino Lotus's digital signal interface D7, Connect the Hall sensor to Seeeduino Lotus's digital signal interface D4. 
   - The microcontroller is then connected to the computer via a USB cable.
- Software code: 
   - Open Arduino IDE. 
   - You can copy the code directly to Arduinoied. Although we encourage you to type the code out by hand to develop your skills.
  ...
  // constants won't change. They're used here to set pin numbers:
  const int HallPin = 4;     // the number of the Hall Sensor pin
...
### 1. Music dynamic rhythm lamp
 ...
 - Hardware connection:
   - Module connection:
     - The default onboard interface specifies:
     - Connect Grove LED to Seeeduino Lotus's digital signal interface D6, connect Buzzer to Seeeduino Lotus's digital signal interface D7.
   - The microcontroller is then connected to the computer via a USB cable.
 - Software code: 
   - Open Arduino IDE. 
   - You can copy the code directly to Arduinoied. Although we encourage you to type the code out by hand to develop your skills.
...
### 2. Make an intelligent sound-light induction desk lamp
 ...
 - Materials: Seeeduino Lotus, Grove LED, Grove cable, Light Sensor, Sound Sensor
  - Hardware connection:
    - Module connection:
      - The default onboard interface specifies:
      - Connect the Grove LED to Seeeduino Lotus's digital signal interface D6, Connect the Light Sensor to Seeeduino Lotus's analog signal interface A1. Connect the Sound Sensor to Seeeduino Lotus's analog signal interface A2 using a Grove cable.
    - The microcontroller is then connected to the computer via a USB cable.
 - Software code: 
   - Open Arduino IDE. 
   - You can copy the code directly to Arduinoied. Although we encourage you to type the code out by hand to develop your skills.
